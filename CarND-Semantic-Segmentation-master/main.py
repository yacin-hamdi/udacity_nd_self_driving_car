...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    skip_connect_1 = tf.add(deconv_layer7, conv_1x1_layer4)
    
    dconv_skip_connect_1 = tf.layers.conv2d_transpose(skip_connect_1, num_classes, 4, 2, padding="SAME",
                                                     kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    conv_1x1_layer3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding="SAME", 
                                      kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    skip_connect_2 = tf.add(dconv_skip_connect_1, conv_1x1_layer3)
    
    output = tf.layers.conv2d_transpose(skip_connect_2, num_classes, 16, 8, padding="SAME", 
                                       kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def run():
    ...
    with tf.Session() as sess:
        ...
        loss_per_epoch = train_nn(sess, epochs, BATCH_SIZE, get_batches_fn, train_op, cross_entropy_loss, input_image,
             correct_label, keep_prob, learning_rate)
