...
double HBF::heuristic(double x1, double y1, double x2, double y2) {
...
vector<HBF::maze_s> HBF::expand(HBF::maze_s state, vector<int> goal) {
    ...
    int g2 = g + 1;
    double f = g2 + heuristic(x, y, goal[0], goal[1]);
...
HBF::maze_path HBF::search(vector< vector<int> > grid, vector<double> start, vector<int> goal) {
    ...
    */
    ...
    double f = g + heuristic(state.x, state.y, goal[0], goal[1]);
    state.f = f;
    ...
    while (!opened.empty())
    {
        ...
        std::sort(opened.begin(), opened.end(), [](maze_s& lhs, maze_s& rhs) {
            return lhs.f < rhs.f;
            });
