 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
     ...
     */
      ...
      double w_epsi = 1000;
      double w_v = 100;
      double w_delta = 500000;
      double w_a = 100;

      for (int i = 0; i < N; i++) {
          fg[0] += w_cte * CppAD::pow(vars[cte_start + i] - p_ref, 2);
          fg[0] += w_epsi * CppAD::pow(vars[epsi_start + i] - psi_ref, 2);
          fg[0] += w_v * CppAD::pow(vars[v_start + i] - v_ref, 2);
      }
      ...
      for (int i = 0; i < N - 2; i++) {
          ...
          fg[0] += CppAD::pow(vars[a_start + i + 1] - vars[delta_start + i], 2);
      }
      ...
      for (int i = 1; i < N; i++) {
          ...
          AD<double> pside0 = CppAD::atan(coeffs[1] + 2 * x0 * coeffs[2] + 3 * 
          ...
          fg[1 + epsi_start + i] = epsi1 - ((psi0 - pside0) - (v0 / Lf) * delta0 * dt);
      }
